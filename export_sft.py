...
NOISE_TEXTS = {
    ...
    "you", "you.", ".", "", "thank you for watching", "thanks for watching",
...
def main():
    parser = argparse.ArgumentParser(description="Export APRIL semantic records to SFT training format.")
    parser.add_argument("--output", "-o", default=str(STATE_DIR / "sft_export.jsonl"), help="Output JSONL file path")
    parser.add_argument("--format", "-f", choices=["chat", "alpaca"], default="chat", help="SFT format (default: chat)")
    parser.add_argument("--min-confidence", type=float, default=0.6, help="Minimum confidence threshold (default: 0.6)")
    parser.add_argument("--success-only", action="store_true", help="Only include successful outcomes")
    parser.add_argument("--single-turn", action="store_true", help="Don't group by session (single-turn only)")
    ...
    filtered = filter_records(records, min_confidence=args.min_confidence, require_success=args.success_only)
    ...
    if args.format == "chat":
        output = export_chat_format(filtered, system_prompt, multi_turn=not args.single_turn)
    else:
