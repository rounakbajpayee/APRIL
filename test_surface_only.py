"""test_surface_only.py
Full main.py import chain, surface system only, no widget.py.
Run from the april directory with: .venv\Scripts\python.exe test_surface_only.py
"""
import sys
import os
import traceback
...
print(f"[test] anchor shown — winId={int(anchor.winId())} pos={anchor.pos()} size={anchor.size()} visible={anchor.isVisible()}")
