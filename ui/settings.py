...
from PyQt6.QtWidgets import (
    ...
    QStackedWidget, QFrame, QScrollArea, QLineEdit,
    QCheckBox, QSlider, QComboBox, QSizePolicy, QApplication
)
...
def _categories_for_mode(mode: APRILMode) -> list[dict]:
    all_cats = [
        dict(id="general",      label="General",      desc="Core APRIL operational behaviour",
             modes=[APRILMode.AMBIENT, APRILMode.FOCUS, APRILMode.TACTICAL]),
        dict(id="voice",        label="Voice",        desc="Speech pipeline and conversational interaction",
             modes=[APRILMode.AMBIENT, APRILMode.FOCUS, APRILMode.TACTICAL]),
        dict(id="intelligence", label="Intelligence", desc="Inference and reasoning configuration",
             modes=[APRILMode.FOCUS, APRILMode.TACTICAL]),
        dict(id="nodes",        label="Nodes",        desc="Distributed orchestration infrastructure",
             modes=[APRILMode.FOCUS, APRILMode.TACTICAL]),
        dict(id="integrations", label="Integrations", desc="External ecosystem integration",
             modes=[APRILMode.FOCUS, APRILMode.TACTICAL]),
        dict(id="behaviors",    label="Behaviors",    desc="Behavior-layer tuning",
             modes=[APRILMode.FOCUS, APRILMode.TACTICAL]),
        dict(id="diagnostics",  label="Diagnostics",  desc="Runtime introspection and orchestration visibility",
             modes=[APRILMode.TACTICAL]),
        dict(id="system",       label="System",       desc="Low-level runtime and internal system controls",
             modes=[APRILMode.TACTICAL]),
    ]
...
class SettingsPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        rail.setStyleSheet("background: transparent; border-right: 1px solid rgba(255,255,255,15);")
        ...
        title.setStyleSheet("color: rgb(165,243,252); font-size: 14px; font-family: 'Inter','Segoe UI'; font-weight: 300; border: none;")
        self._mode_lbl = QLabel(self._core.mode.name.capitalize() + " mode")
        self._mode_lbl.setStyleSheet("color: rgb(113,113,122); font-size: 10px; font-family: 'JetBrains Mono',Consolas; border: none;")
        ...
        search_wrap.setStyleSheet("border-bottom: 1px solid rgba(255,255,255,8); background: transparent;")
        ...
        footer.setStyleSheet("border-top: 1px solid rgba(255,255,255,15); background: transparent;")
        ...
        self._content_hdr.setStyleSheet("border-bottom: 1px solid rgba(255,255,255,15); background: transparent;")
        ch_lay = QHBoxLayout(self._content_hdr)
        ch_lay.setContentsMargins(28, 14, 20, 14)
        self._content_title = QLabel("—")
        self._content_title.setStyleSheet("color: rgb(165,243,252); font-size: 16px; font-family: 'Inter','Segoe UI'; font-weight: 300; border: none;")
        self._content_desc  = QLabel("")
        self._content_desc.setStyleSheet("color: rgb(113,113,122); font-size: 11px; font-family: 'Inter','Segoe UI'; border: none;")
...
def _page_intelligence() -> QWidget:
    ...
    lay.addWidget(_combo_row("Primary model", ["cloud-gemini-flash", "cloud-deepseek-v3", "local-qwen-7b"]))
...
def _page_integrations() -> QWidget:
    ...
    for name, connected in [("Oracle (knowledge store)", True), ("Vikunja (tasks)", True),
                              ("n8n (connector)", True), ("Telegram bot", True), ("Discord", False)]:
...
def _page_behaviors() -> QWidget:
    ...
    lay.addWidget(_combo_row("On new critical notification", ["Pause and notify", "Queue", "Ignore"]))
...
def _toggle(label: str, default: bool) -> QWidget:
    ...
    lay.setContentsMargins(0, 0, 0, 0)
    lbl = QLabel(label)
    lbl.setStyleSheet("color: rgb(200,220,240); font-size: 12px; font-family: 'Inter','Segoe UI';")
    lay.addWidget(lbl)
    lay.addStretch()
    cb = QCheckBox()
...
def _slider_row(label: str, default: int) -> QWidget:
    ...
    val_lbl.setStyleSheet("color: rgb(34,211,238); font-size: 11px; font-family: 'JetBrains Mono',Consolas;")
...
def _info_row(label: str, value: str) -> QWidget:
    ...
    lay.addStretch()
    val = QLabel(value)
    val.setStyleSheet("color: rgb(113,113,122); font-size: 11px; font-family: 'JetBrains Mono',Consolas;")
    lay.addWidget(val)
    return row
