...
def transcribe_with_metadata(
    audio_bytes: bytes, config: dict[str, Any]
    ...
    prefer_local = (
        str(config.get("stt_mode", "remote_first") or "remote_first").strip().lower()
        != "remote_first"
    )
    ...
    if prefer_local:
        ...
        if whisper_host:
            attempts.append(
                (
                    "remote",
                    lambda: _transcribe_remote(audio_bytes, whisper_host, config),
                )
            )
    else:
        if whisper_host:
...
def _transcribe_remote(
    audio_bytes: bytes, whisper_host: str, config: dict[str, Any]
) -> str:
    try:
        import requests
    ...
    response = requests.post(
        url, files=files, data=data, timeout=DEFAULT_TIMEOUT_SECONDS
    )
...
def _transcribe_local(audio_bytes: bytes, config: dict[str, Any]) -> str:
    ...
    model_name = (
        str(config.get("stt_local_model", "small.en") or "small.en").strip()
        or "small.en"
    )
...
def _resolve_transcription_meta(source: str, config: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        "stt_source": "local",
        "stt_model": str(
            config.get("stt_local_model", "small.en") or "small.en"
        ).strip()
        or "small.en",
    }
