...
def process(text: str, config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not clean:
        ...
        plan["_routing"] = {
            ...
            "planner_reason": "rewriter_returned_empty",
        }
        return plan

    # Capture enriched context for training data recording
    _state_context = get_prompt_context_summary(
        limit=int(config.get("state_context_timeline_limit", 6))
    )
    ...
    if local_reply:
        ...
        plan["_enriched_context"] = {
            "memory": _memory_context or "",
            "state": _state_context or "",
        }
        plan["_routing"] = {
            "planner_source": "local_reply",
            "planner_reason": "handled_without_intent_planning",
        }
        return plan

    local_plan = _local_plan(clean, config)
    if local_plan:
        local_plan["_enriched_context"] = {
            "memory": _memory_context or "",
            "state": _state_context or "",
        }
        return local_plan
    ...
    if _looks_like_conversation_question(clean):
        ...
        plan["_routing"] = {
            ...
            "planner_reason": "question_pattern",
        }
        return plan

    llm_plan = _ollama_intent_plan(clean, config)
    if llm_plan:
        llm_plan["_enriched_context"] = {
            "memory": _memory_context or "",
            "state": _state_context or "",
        }
        return llm_plan
    ...
    plan["_routing"] = {
        "planner_source": "conversation_fallback",
        "planner_reason": "no_other_planner_matched",
    }
    return plan
...
def summarize_output(
    raw_output: str, original_request: str, config: dict[str, Any]
) -> str:
...
def _ollama_chat(text: str, config: dict[str, Any]) -> str:
    state_context = get_prompt_context_summary(
        limit=int(config.get("state_context_timeline_limit", 6))
    )
    if state_context:
...
def _ollama_chat_payload(
    user_content: str, config: dict[str, Any], system_content: str
) -> dict[str, Any]:
    try:
        import requests
...
def _local_reply(text: str, config: dict[str, Any]) -> str:
    ...
    if any(
        marker in lowered
        for marker in ("what did you hear", "what did you transcribe", "what did i say")
    ):
    ...
    if any(
        marker in lowered
        for marker in ("what are your open loops", "what is still unresolved")
    ):
        snapshot = load_snapshot()
        open_loops = (
            snapshot.get("open_loops", []) if isinstance(snapshot, dict) else []
        )
        if open_loops:
...
def _load_prompt_files(config: dict[str, Any]) -> str:
    prompt_dir_name = (
        str(config.get("brain_prompt_dir", "prompts") or "prompts").strip() or "prompts"
    )
    ...
    if isinstance(configured_files, str):
        ...
        filenames = [
            str(item).strip() for item in configured_files if str(item).strip()
        ]
    else:
