...
def execute(
    ...
    try:
        result = execute_session_command(
            node, command, config, timeout=int(config.get("shell_timeout_seconds", 20))
        )
    ...
    if not result.get("ok"):
        error_kind = (
            "shell_timeout"
            if int(result.get("returncode", 0) or 0) == 124
            else "shell_failed"
        )
        if output:
            return {
...
def infer_command(text: str, node: str) -> str:
    ...
    if any(
        phrase in lowered
        for phrase in ("current directory", "working directory", "where am i")
    ):
