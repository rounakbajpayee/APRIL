...
OLLAMA_DESCRIPTION = (
    "Update APRIL configuration, voice settings, or teach phrase rewrites"
)
EXAMPLES = [
    {
        "text": "turn off voice",
...
def match(text: str, lowered: str) -> IntentPlan | None:
    teach_match = re.match(
        r"(?:learn that|remember that)\s+(.+?)\s+means\s+(.+)", text, re.IGNORECASE
    )
    if not teach_match:
        teach_match = re.match(
            r"when i say\s+(.+?),?\s+(?:do|mean)\s+(.+)", text, re.IGNORECASE
        )
    if teach_match:
    ...
    if "switch to sapi" in lowered or "use sapi" in lowered:
        ...
        "switch to auto voice" in lowered
        or "use auto voice" in lowered
        or "switch to auto" in lowered
    ):
