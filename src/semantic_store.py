...
def semantic_recall(
    ...
    matches.sort(
        key=lambda item: (item["score"], _record_ts(item["record"])), reverse=True
    )
    return matches[:limit]


def semantic_plan(
    text: str, *, kind: str = "", confidence_threshold: float = 0.74
) -> dict[str, Any] | None:
...
def _normalize_record(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "ts": _clean_field(payload.get("ts", ""))
        ...
        "normalized_text": _clean_field(payload.get("normalized_text", ""))
        or _normalize_text(str(payload.get("text", "") or "")),
...
def _score_match(query: str, query_tokens: list[str], record: dict[str, Any]) -> float:
    ...
    intersection = sum(
        min(query_counter[token], record_counter[token])
        for token in query_counter.keys() & record_counter.keys()
    )
    ...
    prefix_bonus = (
        0.1 if normalized.startswith(query[: min(len(query), len(normalized))]) else 0.0
    )
