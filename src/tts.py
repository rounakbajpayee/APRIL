...
def speak(
    text: str, config: dict[str, Any], on_done: Callable[[bool], None] | None = None
) -> bool:
    ...
    threading.Thread(
        target=_speak_blocking, args=(clean, dict(config), on_done), daemon=True
    ).start()
    return True
...
def _speak_say(text: str, config: dict[str, Any]) -> None:
    if os.name == "nt":
        ...
        execute_session_command(
            node, command, config, timeout=int(config.get("tts_timeout_seconds", 20))
        )
        return
